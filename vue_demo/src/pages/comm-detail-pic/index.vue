<template>
    ...
    <i-search ref="iSearch" :model="formData">
      <el-form-item  prop="HomePicState">
        <el-select v-model="formData.CommId" placeholder="请选择商品">
          <el-option v-for="item in CommIdList" :key="item.CommId" :label="item.CommName" :value="item.CommId"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item  prop="HomePicState">
        <el-select v-model="formData.CommImageState" placeholder="请选择状态">
          <el-option v-for="item in CommImageStateList" :key="item.value" :label="item.label" :value="item.value"></el-option>
        </el-select>
      </el-form-item>
    </i-search>
    ...
    :selectionShow="true" @selection-change="handleSelectionChange">
      <el-table-column v-for="(item, index) in columnlist" :label="item.label" :prop="item.prop" :key="index" align="center" :formatter="columnFormatter">
        ...
        <img v-if="item.prop === 'CommImageUrl'" type= "text" @click="watchImg(scope.row)" :src="scope.row.CommImageUrl" alt="" style="cursor: pointer; width: 50px">
    ...
    <i-dialog :title="dialogTitle" v-model="dialogVisible" @dialog-confirm="dialogConfirm" @dialog-cancel="dialogCancel" @dialog-before-close="dialogBeforeClose">
      <el-form :rules="formRules" ref="form"  :model="dialogFromData" label-width="100px">
                <el-row>
                  <el-col :span="12">
                <el-form-item label="选择图片" class="form-item" prop="CommImageUrl">
                    <!-- <el-input v-model="dialogFromData.HomePicUrl"></el-input> -->
                    <i-file v-model="dialogFromData.CommImageUrl"></i-file>
                </el-form-item>
                  </el-col>
                  <el-col :span="12">
                <el-form-item label="状态" class="form-item" prop="CommImageState">
                    <el-select v-model="dialogFromData.CommImageState" placeholder="请选择">
                      <el-option v-for="item in CommImageStateList" :key="item.value" :label="item.label" :value="item.value"></el-option>
                    </el-select>
                </el-form-item>
                  </el-col>
...
import qer from '../../api/comm-detail-pic'
...
export default {
  ...
  data () {
    return {
          ...
          func: () => {
            if (this.selectDataarray === '') {
              ...
              for (var i in this.selectDataarray) {
                array.push(this.selectDataarray[i].CommImageId)
              }
              // console.log(array)
              // 删除用户发送请求
      ...
      columnlist: [
        {label: '详情Id', prop: 'CommImageId'},
        {label: '商品Id', prop: 'CommId'},
        {label: '商品名称', prop: 'CommName'},
        {label: '预览', prop: 'CommImageUrl'},
        {label: '状态', prop: 'CommImageState', distName: 'CommImageStateList'}
      ],
      homecolumnlist: [
  ...
  methods: {
    reachBtn () {
      ...
      qer('reachViewPageData', {
        ...
        CommImageState: this.formData.CommImageState,
      ...
      this.$refs.homeform.validate((valid) => {
        if (valid) {
          ...
          if (this.homeselectDataarray.length === 1) {
            ...
            this.dialogFromData.CommName = this.homeselectDataarray[0].CommName
